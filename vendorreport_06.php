...
WHERE    a.ContractingModel IN ('連工帶料(UH)', '材料買賣(BH)')
    AND (
        (a.status1 = '已簽約')
        OR
        (a.status1 = '未簽約' AND a.status2 = '已回簽')
    )
    AND (
        a.status1 <> '已完工' OR a.status1 IS NULL
    )
ORDER BY 
    a.auto_seq;";
	...
	<table class="table table-bordered border-dark w-100">
		<thead class="table-light border-dark">
			<tr style="border-bottom: 1px solid #000;">
				...
				<th class="text-center text-nowrap vmiddle" style="width:5%;padding: 10px;background-color: #D4F8D4;">材料用量(M2)</th>
				<th class="text-center text-nowrap vmiddle" style="width:5%;padding: 10px;background-color: #D4F8D4;">志特報價</th>
				<th class="text-center text-nowrap vmiddle" style="width:5%;padding: 10px;background-color: #D4F8D4;">請志特報價日期</th>
...
EOT;
    ...
    while ($row=$mDB->fetchRow(2)) {
		...
		$fmt_material_usage = number_format($material_usage);
		//志特報價
		$geto_quotation = $row['geto_quotation'];
		$fmt_geto_quotation = number_format($geto_quotation);
		$SUM_geto_quotation += $geto_quotation;
		//請志特報價日期
...
$casereport_list.=<<<EOT
			<tr>
				...
				<th class="text-center" style="width:5%;padding: 10px;">$fmt_std_layer_template_qty</th>
				<th class="text-center" style="width:5%;padding: 10px;">$fmt_roof_protrusion_template_qty</th>
				<th class="text-center" style="width:5%;padding: 10px;">$fmt_material_usage</th>
				<th class="text-center" style="width:5%;padding: 10px;">$fmt_geto_quotation</th>
...
EOT;
	...
	$fmt_SUM_geto_quotation = number_format($SUM_geto_quotation);
...
$casereport_list.=<<<EOT
	...
	<tfoot>
			<tr>
				...
				<th class="text-center" style="width:5%;padding: 10px;background-color: #FFF2CC; font-weight: bold; font-size: 16px;"></th>
				<th class="text-center" style="width:5%;padding: 10px;background-color: #FFF2CC; font-weight: bold; font-size: 16px;">$fmt_SUM_geto_quotation</th>
				<th class="text-center" style="width:5%;padding: 10px;background-color: #FFF2CC"></th>
				<th class="text-center text-nowrap" style="width:5%;padding: 10px;background-color: #FFF2CC"></th>
				<th class="text-center text-nowrap" style="width:5%;padding: 10px;background-color: #FFF2CC"></th>
			</tr>
